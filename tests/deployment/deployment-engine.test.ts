...
test('failed post-deploy service verification marks deployment and task failed before health success',async()=>{const {temp,db,tasks,engine,task}=await setup(false);try{const result=await engine.deploy(task.taskId,'project-a',principal);assert.equal(result.status,'FAILED');assert.equal(result.healthCheck,null);assert.match(JSON.stringify(result.error),/service state is failed/i);assert.equal(tasks.get(task.taskId)?.status,'FAILED');assert.equal(result.rollbackAvailable,true);}finally{db.close();await temp.cleanup();}});
