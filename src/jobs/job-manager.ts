...
export class JobManager {
  ...
  public async start(projectId:string,principal:Principal,commandId:string,taskId?:string):Promise<JobRecord>{
    ...
    const execution=this.runner.run(projectId,principal,commandId,{signal:controller.signal,onSpawn:(pid)=>this.db.raw.prepare('UPDATE jobs SET pid=? WHERE job_id=?').run(pid??null,id)})
      .then(async result=>{await fs.writeFile(stdoutPath,result.stdout,{mode:0o640});await fs.writeFile(stderrPath,result.stderr,{mode:0o640});const status:JobStatus=controller.signal.aborted?'CANCELLED':result.exitCode===0?'COMPLETED':'FAILED';this.db.raw.prepare('UPDATE jobs SET status=?,exit_code=?,finished_at=? WHERE job_id=?').run(status,result.exitCode,new Date().toISOString(),id);if(taskId!==undefined)this.appendTaskCommand(taskId,{jobId:id,commandId,status,exitCode:result.exitCode});})
      ...
      .finally(()=>{this.controllers.delete(id);this.active.delete(id);});
