...
function registerOperationalTools(registry: McpToolRegistry, services: ServerAgentMcpServices): void {
  ...
  registry.register({ definition: { name: 'deploy', description: 'Start guarded deployment as a durable operation and return operation_id immediately.', inputSchema: objectSchema({ project_id: projectIdSchema, task_id: stringSchema }, ['project_id', 'task_id']) }, permission: 'deploy:run', projectArgument: 'project_id', mutating: true, handler: async (args, ctx) => {
...
function registerRecoveryTools(registry: McpToolRegistry, services: ServerAgentMcpServices): void {
  ...
  registry.register({ definition: { name: 'rollback_execute', description: 'Start execution of a READY rollback plan as a durable operation after revalidation.', inputSchema: objectSchema({ project_id: projectIdSchema, rollback_id: stringSchema }, ['project_id', 'rollback_id']) }, permission: 'rollback:run', projectArgument: 'project_id', mutating: true, handler: async (args, ctx) => {
