...
export function registerGitCommandTools(registry:McpToolRegistry,git:GitService,jobs:JobManager,validation:LocalValidationPipeline,operations:PersistentOperationManager):void{
  ...
  registry.register({definition:{name:'git_commit',description:'Create a Git commit containing only explicitly listed project paths. Existing unrelated staged changes are excluded.',inputSchema:objectSchema({project_id:projectIdSchema,message:stringSchema,paths:{type:'array',items:{type:'string'},minItems:1,maxItems:256}},['project_id','message','paths'])},permission:'git:write',projectArgument:'project_id',handler:async(args,ctx)=>git.commit(stringArg(args,'project_id')??'',ctx.principal,stringArg(args,'message',{max:200})??'',stringArrayArg(args,'paths')??[])});
  registry.register({definition:{name:'run_command',description:'Start one project-registered command as a persistent bounded job. Optional task_id links cancellation and task state to the job.',inputSchema:objectSchema({project_id:projectIdSchema,command_id:stringSchema,task_id:stringSchema},['project_id','command_id'])},permission:'commands:run',projectArgument:'project_id',handler:async(args,ctx)=>jobs.start(stringArg(args,'project_id')??'',ctx.principal,stringArg(args,'command_id',{max:128})??'',stringArg(args,'task_id',{optional:true,max:128}))});
  registry.register({definition:{name:'run_tests',description:'Start the project registered test command as a persistent bounded job. Optional task_id links it to a running task.',inputSchema:objectSchema({project_id:projectIdSchema,task_id:stringSchema},['project_id'])},permission:'commands:run',projectArgument:'project_id',handler:async(args,ctx)=>jobs.start(stringArg(args,'project_id')??'',ctx.principal,'test',stringArg(args,'task_id',{optional:true,max:128}))});
  registry.register({definition:{name:'run_validation',description:'Start project Local CI as a durable operation and return operation_id immediately.',inputSchema:objectSchema({project_id:projectIdSchema,task_id:stringSchema},['project_id','task_id'])},permission:'commands:run',projectArgument:'project_id',handler:async(args,ctx)=>{
