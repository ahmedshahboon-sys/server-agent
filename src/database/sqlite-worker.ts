...
import type { DatabaseMigrationStatus, DatabaseQueryRequest, DatabaseQueryResult, DatabaseSchemaResult, DatabaseStatusResult } from './adapter.js';
...
try{
  let result:DatabaseStatusResult|DatabaseSchemaResult|DatabaseQueryResult|readonly DatabaseQueryResult[]|DatabaseMigrationStatus;
  if(input.operation==='status'){db.prepare('SELECT 1 AS ok').get();result={connected:true,latencyMs:Date.now()-started,details:{adapter:'sqlite'}};}
  else if(input.operation==='schema'){const rows=db.prepare("SELECT type,name,tbl_name AS tableName,sql FROM sqlite_master WHERE name NOT LIKE 'sqlite_%' ORDER BY type,name LIMIT ?").all(input.maxRows??500);result={objects:rows.map(objectRow)};}
  else if(input.operation==='query'){if(input.request===undefined)throw new Error('Missing query request');result=runQuery(db,input.request);}
  else if(input.operation==='transaction'){const requests=input.requests??[];if(requests.length===0)throw new Error('Transaction requires statements');const hasWrite=requests.some((request)=>request.classification==='WRITE');db.exec(hasWrite?'BEGIN IMMEDIATE;':'BEGIN;');try{const results:DatabaseQueryResult[]=[];for(const request of requests)results.push(runQuery(db,request));db.exec('COMMIT;');result=results;}catch(error){db.exec('ROLLBACK;');throw error;}}
  else result=migrationStatus(db);
  parentPort?.postMessage({ok:true,result});
}catch(error){parentPort?.postMessage({ok:false,error:{name:error instanceof Error?error.name:'Error',message:error instanceof Error?error.message:String(error)}});}
finally{db.close();}
