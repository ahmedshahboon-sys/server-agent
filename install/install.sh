...
if [[ ! -e "${ENV_FILE}" ]]; then
  ...
  echo "Created ${ENV_FILE} from the public example. Replace the bearer-token placeholder before enabling the service."
else
...
if (( ENABLE_SERVICE == 1 )); then
  ...
  echo "Installation package prepared successfully. Service was NOT started or enabled."
  echo "After reviewing ${ENV_FILE}, explicitly run: sudo ${SOURCE_DIR}/install/install.sh --enable"
fi
