...
if (!envExample.includes('SERVER_AGENT_OPERATIONAL_RETENTION_DAYS=30')) errors.push('operational retention must be configured');
if (!envExample.includes('SERVER_AGENT_JOB_LOG_RETENTION_DAYS=14')) errors.push('job log retention must be configured');
if (!envExample.includes('SERVER_AGENT_MIN_FREE_DISK_BYTES=268435456')) errors.push('minimum free disk guard must be configured');
if (!envExample.includes('SERVER_AGENT_STATE_DB_WARNING_BYTES=536870912')) errors.push('state DB warning threshold must be configured');
if (!envExample.includes('SERVER_AGENT_MAINTENANCE_MODE=false')) errors.push('maintenance mode must default to disabled');
